...
describe('RelationshipRegistry', () => {
  ...
  describe('Basic Operations', () => {
    it('should create a relationship with a unique ID', () => {
      ...
      const result = registry.createRelationship(params);
    ...
    it('should retrieve a relationship by ID', () => {
      const params: CreateRelationshipParams = {
      ...
      const result = registry.createRelationship(params);
      const retrieved = registry.getRelationship(result.relationship!.id);

      expect(retrieved).toEqual(result.relationship);
    });

    it('should retrieve relationships from a source holon', () => {
      ...
      const relationships = registry.getRelationshipsFrom(sourceHolonID);
      expect(relationships).toHaveLength(2);
    });

    it('should retrieve relationships to a target holon', () => {
      ...
      const relationships = registry.getRelationshipsTo(targetHolonID);
      expect(relationships).toHaveLength(2);
    });
    ...
    it('should filter relationships by type', () => {
      ...
      const occupiesRels = registry.getRelationshipsFrom(sourceHolonID, RelationshipType.OCCUPIES);
      expect(occupiesRels).toHaveLength(1);
      expect(occupiesRels[0].type).toBe(RelationshipType.OCCUPIES);
    });

    it('should end a relationship', () => {
      const result = registry.createRelationship({
        type: RelationshipType.OCCUPIES,
        sourceHolonID: 'person-1',
        targetHolonID: 'position-1',
        properties: {},
      ...
      const endResult = registry.endRelationship({
      ...
      const updated = registry.getRelationship(result.relationship!.id);
      expect(updated!.effectiveEnd).toEqual(new Date('2024-06-01'));
    });

    it('should not allow ending a relationship with end date before start date', () => {
      const result = registry.createRelationship({
        type: RelationshipType.OCCUPIES,
        sourceHolonID: 'person-1',
        targetHolonID: 'position-1',
    ...
    it('should filter relationships by effective date', () => {
      const result1 = registry.createRelationship({
        type: RelationshipType.OCCUPIES,
        sourceHolonID: 'person-1',
        targetHolonID: 'position-1',
        properties: {},
      ...
      const result2 = registry.createRelationship({
        type: RelationshipType.OCCUPIES,
        sourceHolonID: 'person-1',
      ...
      const activeInMarch = registry.getRelationshipsFrom('person-1', undefined, {
      ...
      const activeInAugust = registry.getRelationshipsFrom('person-1', undefined, {
  ...
  describe('Event Generation', () => {
    it('should generate an event when creating a relationship', () => {
      const result = registry.createRelationship({
        type: RelationshipType.OCCUPIES,
        sourceHolonID: 'person-1',
        targetHolonID: 'position-1',
    ...
    it('should generate an event when ending a relationship', () => {
      const result = registry.createRelationship({
        type: RelationshipType.OCCUPIES,
        sourceHolonID: 'person-1',
        targetHolonID: 'position-1',
   ...
   */
  it('Property 6: Relationship completeness', () => {
    fc.assert(
      fc.property(
        ...
        (params) => {
          ...
          const result = registry.createRelationship(params as CreateRelationshipParams);
   ...
   */
  it('Property 8: Relationship change event generation', () => {
    fc.assert(
      fc.property(
        ...
        ({ createParams, endParams }) => {
          ...
          // Create relationship
          const createResult = registry.createRelationship(createParams as CreateRelationshipParams);
