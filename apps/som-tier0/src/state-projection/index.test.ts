...
describe('State Projection Engine - Property-Based Tests', () => {
  ...
  describe('Property 13: State derivation from events', () => {
    it('should derive correct state from a sequence of holon creation events', async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          (holonIds, timeOffsets, eventConfigs) => {
            return (async () => {
              ...
              const state = await projectionEngine.replayAllEvents();
    ...
    it('should apply events in chronological order regardless of submission order', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid(),
          ...
          (holonId, eventCount) => {
            return (async () => {
              ...
              const eventStore1 = new InMemoryEventStore();
              ...
              for (const event of events) {
                await eventStore1.submitEvent(event);
              }

              // Submit events in reverse order to store 2
              for (let i = events.length - 1; i >= 0; i--) {
                await eventStore2.submitEvent(events[i]);
              }
              ...
              const state1 = await engine1.replayAllEvents();
              const state2 = await engine2.replayAllEvents();
    ...
    it('should correctly track modification events for each holon', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid(),
    ...
    it('should handle relationship creation and end events correctly', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid(),
          fc.uuid(),
    ...
    it('should handle holon deactivation events correctly', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid(),
          (holonId) => {
    ...
    it('should produce consistent state when replaying the same events multiple times', async () => {
      await fc.assert(
        fc.asyncProperty(
          ...
          (holonIds) => {
            return (async () => {
              ...
              const state1 = await engine1.replayAllEvents();
              const state2 = await engine2.replayAllEvents();
              const state3 = await engine3.replayAllEvents();
    ...
    it('should correctly handle incremental state updates with new events', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.array(fc.uuid(), { minLength: 5, maxLength: 15 }),
          fc.integer({ min: 1, max: 10 }),
          (holonIds, splitIndex) => {
            return (async () => {
              ...
              // Replay first batch
              await projectionEngine.replayAllEvents();
              ...
              for (let i = 0; i < secondBatch.length; i++) {
                ...
                const fullEvent = await eventStore.getEvent(eventId);
                if (fullEvent) {
    ...
    it('should respect event validity windows when projecting state', async () => {
      await fc.assert(
        fc.asyncProperty(
          fc.uuid(),
          (holonId) => {
            return (async () => {
              ...
              const stateBefore = await projectionEngine.replayEventsAsOf(new Date(baseTime.getTime() + 2000));

              // Query during validity window
              const stateDuring = await projectionEngine.replayEventsAsOf(new Date(baseTime.getTime() + 5000));

              // Query after validity window
              const stateAfter = await projectionEngine.replayEventsAsOf(new Date(baseTime.getTime() + 8000));
