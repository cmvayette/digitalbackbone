...
export interface OwnerPickerProps {
  value: string;
  onChange: (ownerId: string, ownerType?: string, ownerName?: string) => void;
  onClose: () => void;
  className?: string;
}
...
export const OwnerPicker: React.FC<OwnerPickerProps> = ({ value, onChange, onClose, className }) => {
  ...
  const candidates: OwnerOption[] = getCandidates().map(c => ({
    ...c,
    type: c.type === 'Organization' ? 'Organization' : 'Position'
  }));

  // Add Agents (still mocked for now)
  ...
  const handleSelect = (opt: OwnerOption) => {
    onChange(opt.id, opt.type, opt.name);
    onClose();
  };

  return (
    <div className={`w-64 bg-slate-800 border border-slate-700 rounded-lg shadow-xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-100 ${className}`}>
