...
export class APIRoutes {
  ...
  async matchPattern(request: APIRequest<PatternMatchRequest>): Promise<APIResponse> {
    ...
    const graphPattern = {
      holonConstraints: pattern.holonTypes?.map(type => ({ type })) || [],
      relationshipConstraints: pattern.relationshipTypes?.map(type => ({ type })) || [],
    };
  ...
  async submitSchemaProposal(request: APIRequest<SchemaProposalRequest>): Promise<APIResponse> {
    ...
    const proposal = this.governance.createSchemaProposal({
      proposalType,
      name,
      description,
      definition,
  ...
  async getSchemaVersions(request: APIRequest<SchemaVersionRequest>): Promise<APIResponse> {
    ...
    const versions = this.schemaVersioning.getSchemaVersions(version, includeDeprecated);
  ...
  async getCurrentSchema(request: APIRequest): Promise<APIResponse> {
    const currentSchema = this.schemaVersioning.getCurrentSchema();
  ...
  async submitExternalData(request: APIRequest<ExternalDataSubmissionRequest>): Promise<APIResponse> {
    ...
    const result = await this.semanticAccessLayer.submitExternalData(externalSystem, {
      externalSystem,
      externalID,
      dataType,
      payload,
  ...
  async queryIDMapping(request: APIRequest<IDMappingRequest>): Promise<APIResponse> {
    ...
    const holonID = this.semanticAccessLayer.mapExternalIDToHolonID(externalSystem, externalID);
